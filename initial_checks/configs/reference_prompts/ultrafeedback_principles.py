# UltraFeedback principle bank -- verbatim from
# github.com/OpenBMB/UltraFeedback, src/comparison_data_generation/main.py.
# Paper: arXiv:2310.01377 (CC BY-SA 4.0). Repo licence: check before redistributing.
#
# USE IN THE ORIGINAL PIPELINE: one principle is sampled per completion and
# "added to the system prompt to induce model behaviors" (paper section 2.3), then the
# completion is scored by GPT-4. 255,864 completions over 63,967 instructions.
# UltraFeedback trained Zephyr and is one of the most-used open preference sets.
#
# PARTIAL MATCH ONLY. The generation half is ours -- short principle in the system
# prompt, stripped afterwards. The training half is not: UltraFeedback builds
# *preference* pairs for DPO, not SFT on filtered outputs. Cite it for the prompts,
# not as a precedent for the pipeline.
#
# 61 principles across 5 categories. 'harmlessness' is the relevant one for this
# project; 'honesty' and 'truthfulness' are candidate secondary axes.
